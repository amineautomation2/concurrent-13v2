...
class SupabaseQueueManager:
    ...
    def fetch_and_lock_task(self, runner_id: str, fund_type: str) -> dict | None:
        ...
        try:
            response = self.client.table("aviva_scraping_queue") \
            ...
            self.client.table("aviva_scraping_queue") \
                .update({"status": "processing", "locked_by": runner_id, "updated_at": "now()"}) \
                .eq("id", task["id"]) \
                .execute()
    ...
    def update_task_status(self, task_id: int, status: str, error_inc: bool = False):
        ...
        if error_inc:
            ...
            pass
        self.client.table("aviva_scraping_queue").update(update_data).eq("id", task_id).execute()
    ...
    def stream_funds_batch(self, fund_type: str, records: list[dict]):
        ...
        self.client.table("aviva_funds_data").upsert(payloads, on_conflict="url").execute()