...
class SupabaseSeeder:
    def __init__(self):
        ...
        if not url or not key:
            print(
                "❌ Error: SUPABASE_URL and SUPABASE_KEY environment variables must be set.", file=sys.stderr)
            sys.exit(1)
    ...
    def populate_kiid_tasks(self, discovered_funds: list[dict], batch_size: int = 200):
        ...
        for i in range(0, len(payload_records), batch_size):
            ...
            try:
                self.client.table("aviva_scraping_queue").insert(
                    chunk).execute()
                ...
                print(
                    f"❌ Failed to insert database batch starting at index {i}: {e}", file=sys.stderr)
...
def transition_to_kiid_queue():
    ...
    logging.info(
        "Searching for master entries with valid KIID URLs missing ISINs...")
   ...
   # 1. Paginate through the master database until all rows are fetched
    while True:
        ...
        response = supabase.table("aviva_funds_data") \
